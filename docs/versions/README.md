...
| 版本 | 发布顺序 | 跨度 |
|------|----------|------|
| rc18 → rc20 | 2 个 RC | 约 1 周 |
| rc20 → rc24 | 4 个 RC | 约 2 周 |
| rc24 → rc25 | 1 个 RC | 2 天 |

### 各阶段变更统计

| 阶段 | Commits | PRs | fix | perf | feat | revert/tools |
|------|---------|------|-----|------|------|--------------|
| **rc18 → rc20** | 54 | 25 | 18 | 3 | 2 | 2 |
| **rc20 → rc24** | 72 | 36 | 21 | 8 | 5 | 2 |
| **rc24 → rc25** | 2 | 1 | 1 | 0 | 0 | 0 |
| **rc20 → rc25** | 74 | 35 | 21 | 8 | 4 | 2 |

---

### rc18 → rc20 (Worker 主要变更)

| 类别 | PRs | 关键修复 |
|------|-----|----------|
| **🔴 高风险** | 2 | 连接重连 (!860)、并发安全 (!874) |
| **🟡 中风险** | 8 | RPC 预热 (!884)、异步删除 (!849)、环境持久化 (!879) |
| **🟢 低风险** | 15 | 日志优化、指标改进、URMA 诊断 |

**主要收益**: 连接稳定性、并发安全、RPC 冷启动优化

---

### rc20 → rc24 (Client + Worker 变更)

| 类别 | PRs | 关键修复 |
|------|-----|----------|
| **🔴 高风险** | 2 | MCreate bug (!859)、时钟偏差 (!933) |
| **🟡 中风险** | 10 | TCP 限流 (!918)、RPC 超时 (!903→!950 revert) |
| **🟢 低风险** | 24 | 日志治理、ZMQ 指标、URMA 增强 |

**主要收益**: 客户端 MCreate bug 修复、跨机房时钟偏差处理、ZMQ 延迟指标

---

### rc24 → rc25 (仅 2 个 Commits)

| PR # | 描述 | 类型 |
|------|------|------|
| !970 | fix: remove PID from dsclient log filename | fix |

**说明**: rc24 到 rc25 只做了一个日志文件名简化修复
...
```
v0.8.1-rc18                    v0.8.1-rc20                    v0.8.1-rc24/rc25
     │                              │                              │
     │   [Worker 重点修复]          │   [Client + Worker 修复]    │   [日志优化]
     │   - 连接重连                 │   - MCreate bug             │   - PID 移除
     │   - 并发安全                 │   - 时钟偏差处理             │
     │   - RPC 预热                 │   - TCP 限流完善             │
     │   - 异步删除                 │   - ZMQ 延迟指标             │
     │   - 环境持久化               │   - 超时参数调整             │
...
| 当前版本 | 目标版本 | 建议 | 关键收益 |
|----------|----------|------|----------|
| rc18 | rc20 | ✅ **强烈推荐** | 连接稳定性、并发安全 |
| rc20 | rc24/25 | ✅ 推荐 | MCreate bug、时钟偏差、ZMQ 指标 |
| rc18+rc20 混合 | rc25 | ✅ **一起升级** | 综合收益最大化 |
...
| 风险项 | 影响版本 | 风险等级 | 状态 |
|--------|----------|----------|------|
| MCreate NX placeholder bug | Client rc18 | 🔴 高 | ✅ 已在 !859 修复 |
| 跨机房时钟偏差 | Client rc18 | 🔴 高 | ✅ 已在 !933 修复 |
| UB 重连问题 | Worker rc18 | 🔴 高 | ✅ 已在 !860 修复 |
| 并发数据竞争 | Worker rc18 | 🔴 高 | ✅ 已在 !874 修复 |
| 超时参数 2ms 过短 | rc20 | 🟡 中 | ✅ 已在 !950 回退到 10ms |
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-05-12 | v3.0 | 添加附录：rc18→rc20→rc24→rc25 版本变更总结 |
