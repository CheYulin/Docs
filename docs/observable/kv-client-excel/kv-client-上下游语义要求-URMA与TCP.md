...
- **写放大**：`MultiPublish` 含 **Worker→Master** 与 **Worker→Worker UB**；扩缩容窗口 **`K_SCALING`** 为 **预期可重试**，不是单一「bug」信号。
...
- **控制面必须先稳定**：若 `stub_->Get` 已失败，不必再分析 UB（读路径分层见 `kv-client-读接口-定位定界.md` §7/§8）。  
