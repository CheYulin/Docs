...
**说明**：`worker2` 表示元数据逻辑对端（Master/etcd 链路），`worker3` 表示数据副本 Worker。
...
**仓库**：`yuanrong-datasystem`。  
**说明**：`worker2` 表示元数据逻辑对端（实现可能是 Master / etcd 链路）；`worker3` 表示数据副本 Worker。
...
| **worker1→worker2** | 入口 Worker 访问 **元数据侧**（实现上常为 Master/etcd/gRPC；填表时 **worker2 = 逻辑对端**，可注实际服务名）。 |
...
| 查元数据 | **worker1→worker2** | `TryGetObjectFromRemote` 前置链<br>`|_ QueryMeta / etcd / Master`（依实现）<br>`worker2`<br>`|_ 返回 meta 与地址` | — | etcd 访问等 syscall | **`K_RPC_UNAVAILABLE`（如 etcd 续约）** 等经 worker1 填响应 | Master 侧常有自有重试 | `etcd is unavailable` |
...
| worker1 元数据与下推 | **worker1**<br>**worker1→worker2** | `worker_oc_service_multi_publish_impl` 等<br>`|_ Master / 元数据提交`<br>`worker2` | — | — | RPC / `last_rc` | Worker 策略 | `scaling`；master fail 文案 |
...
| 文档 | 用途 |
|------|------|
| [kv-client-Sheet2-URMA-C接口映射.md](./kv-client-Sheet2-URMA-C接口映射.md) | URMA C API 映射 |
