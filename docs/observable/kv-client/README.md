...
**工作簿 + Sheet 对照稿（独立目录）** → [`../workbook/kv-client/`](../workbook/kv-client/README.md)（`.xlsx` 与 `kv-client-Sheet*.md` 均在此，便于打包查看）。

**全文按用途梳理** → [`文档索引.md`](./文档索引.md)（评审、改版建议可对照该索引逐类提）。

**客户向：读写全分支 + Trace 粒度 SOP + 细 Mermaid**：[kv-client-定位定界-客户操作手册-分支全量与Trace粒度.md](kv-client-定位定界-客户操作手册-分支全量与Trace粒度.md)

## 精选必读（3-5篇）

- [`kv-client-定位定界手册-基于Excel.md`](./kv-client-定位定界手册-基于Excel.md)
- [`kv-client-定位定界-客户操作手册-分支全量与Trace粒度.md`](./kv-client-定位定界-客户操作手册-分支全量与Trace粒度.md)
- [`kv-client-故障模式库-故障清单-日志与告警.md`](./kv-client-故障模式库-故障清单-日志与告警.md)
- [`../workbook/kv-client/README.md`](../workbook/kv-client/README.md)
- [`puml/README-总图与分图.md`](./puml/README-总图与分图.md)

## 文件

| 文件 | 说明 |
|------|------|
| [../workbook/kv-client/kv-client-观测-调用链与URMA-TCP.xlsx](../workbook/kv-client/kv-client-观测-调用链与URMA-TCP.xlsx) | **Sheet1**：第二列含 **`【故障预期】`**（**无错误**=健康主路径；**可失败**=异常行）、**`【调用链逻辑】`**、多行 **`└─` 树** + case/位置；**正向=调用树**，**逆向=错误码→日志→Sheet5**（见 [`../workbook/kv-client/README.md`](../workbook/kv-client/README.md)）。**Sheet2** OS；**Sheet3** URMA；**Sheet4** 性能；**Sheet5** 定界-case；**Sheet6** URMA 错误码解释 |
| [../workbook/kv-client/kv-client-Sheet1-调用链-错误与日志.md](../workbook/kv-client/kv-client-Sheet1-调用链-错误与日志.md) | Sheet1 展开 + 代码引用 |
| [../workbook/kv-client/kv-client-Sheet2-URMA-C接口映射.md](../workbook/kv-client/kv-client-Sheet2-URMA-C接口映射.md) | URMA 查表展开 + `urma_manager.cpp` 证据 |
| [../workbook/kv-client/kv-client-Sheet3-TCP-RPC对照.md](../workbook/kv-client/kv-client-Sheet3-TCP-RPC对照.md) | TCP / RPC 对照展开 |
| [../workbook/kv-client/README.md](../workbook/kv-client/README.md) | 工作簿目录说明与再生命令 |
| [文档索引.md](./文档索引.md) | 全目录按用途分组索引 |
...
| `./ops docs.kv_observability_xlsx` | 重新生成 xlsx（依赖 `openpyxl`） |

**可靠性 / FEMA 联动（Init·MCreate·MSet·MGet）**：**[使用步骤](../../../workspace/reliability/kv-sdk-fema-使用步骤.md)** · [背景与流程图](../../../workspace/reliability/kv-sdk-fema-reliability-observability.md) · `./ops docs.kv_fema_workbook` → `workspace/reliability/kv_sdk_fema_analysis.xlsx`。
...
| 2026-04-11 | **workbook/**：集中存放 xlsx 与 Sheet 对照 md；新增 **文档索引.md** |
