...
本文约定：**表头下一行起**，**第二列** 在生成器中已扩展为：**`【故障预期】`** + **`【调用链逻辑】`** + **`【调用链树】`**（`|_` → `└─`）+ **`【本行 case】`**。手工填表时可简写，但建议保留 **故障预期**：**无错误**（健康主链）与 **可失败**（异常行）对照。

**第二列「调用链段」** 仍可按 **编号 + 跨进程语义** 书写；同一接口多张表时，**先铺最长路径**，异常分支另起行或另表（见文末）。

**命名**：`worker1` = **入口 Worker**（SDK `stub` 连到的实例）；`worker2` = **Directory（对象目录）分片所在 Worker**（按 **hash ring** 负责一部分 object key 的目录查询；**etcd** 多用于租约/路由而非 RPC 对端本体；填表时可用 **实际进程名/IP** 替换）；`worker3` = **数据所在 Worker**（远端副本侧，发起/参与 UB 数据面的一侧）。源码/API 名可能仍含 `Master`/`QueryMeta`，**日志 grep** 仍可能出现 `Query from master failed` **原文**。
...
| 4 | **3. worker1→worker2：查询对象目录（Directory）** | **②** | `QueryMeta` 等到 **Directory Worker**（hash ring 分片）；etcd 多为租约/路由侧 |
