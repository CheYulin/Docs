...
| 08 | [08-fault-triage-consolidated.md](08-fault-triage-consolidated.md) | **值班用手册**：**§导入** 从 access 日志看成功率与 P99；通断/时延 × 五边界定界；正文 **§6** 全量 54 条 KV metrics；附录为运维速查 | `kv_metrics`（全量表）/ `status.h` / `zmq` / `urma`（细节见 05/07） |
