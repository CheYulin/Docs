...
- L0（集成方）：非法参数、超时配置不合理
- L1（OS/网络）：1002/reset/unreachable、mmap/fd 内核异常
...
5. 若 UB 与 TCP 同时失败，再进入网络/OS 联合排查（L1）。

## 7.4 定界结论规则
- 仅 UB 失败、TCP 可用：优先 UMDK/UB 路径（L1），数据系统给出端点对与失败时段自证。
...
## 8.4 定界结论规则
- SDK 1002 + Worker无入口日志：L1（网络/连接）优先。
- SDK 1002 + Worker有入口且内部超时：L3（Worker处理链）优先，再细分是否远端依赖。
