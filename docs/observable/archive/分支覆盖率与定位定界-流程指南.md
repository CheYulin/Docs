...
本文回答两件事：**如何判断代码分支是否被测试覆盖**（含工程上可行的「尽量全覆盖」做法），以及**用一张总流程图把「覆盖率工作」与「线上/工单定位定界」串起来**。与 KV Client 专用 PlantUML 总图互补：[`../kv-client/puml/kv-client-定位定界-总图.puml`](../kv-client/puml/kv-client-定位定界-总图.puml)。
...
| [`../kv-client/puml/kv-client-定位定界-总图.puml`](../kv-client/puml/kv-client-定位定界-总图.puml) | **更细**的 Init/读/写 PlantUML 分支 |
| [`../kv-client/README.md`](../kv-client/README.md) | 定界文档入口；工作簿位于 `../workbook/kv-client/` |
