...
**方案锚点**：[FAULT_HANDLING_AND_DATA_RELIABILITY.md](../FAULT_HANDLING_AND_DATA_RELIABILITY.md) **§二–§四**。**客户端码与 31/32**：[scaling_scale_down_client_paths.md](./scaling_scale_down_client_paths.md)、Playbook **§4.2 / §4.3**。**建链细节（研发向）**：[CLIENT_WORKER_AND_WORKER_WORKER_LINK_FAILURE_ANALYSIS.md](../CLIENT_WORKER_AND_WORKER_WORKER_LINK_FAILURE_ANALYSIS.md)。
...
| **L4 Worker / Master 日志** | 平台 / 二线 | 在 **已锁定节点** 上拉 **ERROR/WARN**（见下节 **关键词**）；可选拉 **`resource.log`**（`log_monitor`）看 **RPC 池排队、etcd 队列/成功率、SHM** 等与变更对齐（见 [worker_resource_log_triage.md](./worker_resource_log_triage.md)），按 **时间戳** 与变更对齐 | 用于确认 **缩容/迁移/元数据** 是否在报错，以及 **服务端是否排队/控制面是否异常** |
...
| 5 | 仍失败 | 带上 **Status 全文 + 时间线** 升级；研发侧再对 [CLIENT_WORKER…](../CLIENT_WORKER_AND_WORKER_WORKER_LINK_FAILURE_ANALYSIS.md) |
...
- [KV_CLIENT_FAULT_TRIAGE_TREE.md](../KV_CLIENT_FAULT_TRIAGE_TREE.md) 与 Playbook **§4.2**。
